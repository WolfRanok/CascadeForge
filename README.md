...
- 四个象限分别只开放一个目标 Mask，并在下载后本地确定性累积；每轮只新增一个目标，历史编辑不会漂移。
- Mask 外像素由程序恢复为原图或上一轮结果，可清除模型越界生成的编号、标签和提前修改。
- 四个象限仍在同一次图像编辑 API 请求中生成，不会把运行时间或 API 调用数增加到四倍。
...
上图为原创示意图：四个面板表示四个单目标编辑逐轮累积后的效果，不代表仓库内的真实数据样例。
