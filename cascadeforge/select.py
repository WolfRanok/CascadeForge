...
5. short 不超过 10 个汉字；long 为 15–30 个汉字，使用实体名称、颜色和位置明确指出目标。
6. long 中禁止出现“编号”“ID”“候选”“第几轮”等候选图内部标识；编辑图中没有这些编号。
...
def validate_response(data: dict, masks: np.lib.npyio.NpzFile, candidate_meta: list[dict]):
    ...
    for round_index in range(1, 5):
        ...
        if re.search(r"编号|候选|\bID\s*[0-9一二三四五六七八九十]+|第[一二三四1234]轮", value["long"], re.IGNORECASE):
            raise ValueError(f"ROUND_{round_index} 不能引用候选图编号或轮次编号")
...
def make_outputs(
    ...
    # Store one independent target per quadrant. The editor composes these
    # results locally, so the model never has to preserve earlier edits.
    incremental = []
    used = np.zeros(selected[0].shape, dtype=bool)
    ...
    for target, position in zip(incremental, positions):
    ...
    if buffer.tell() > MAX_MASK_SIZE:
        raise ValueError(f"四宫格 Mask 为 {buffer.tell() / 1024 / 1024:.1f} MB，超过 6 MB")
