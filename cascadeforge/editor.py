...
2. 右上：完整保留第一轮结果，只额外执行第二轮——{rounds[1]}
3. 左下：完整保留前两轮结果，只额外执行第三轮——{rounds[2]}
4. 右下：以左下第三轮结果为内容基础，对整个右下象限执行全局变换——{rounds[3]}

硬性要求：
- 前三个象限每轮只新增一个指定物体的编辑，不得提前执行后续轮次。
- 第四象限允许整体改变天气、昼夜、季节、光照、氛围和色调，但必须保留第三轮中的场景布局、主体位置以及前三个已编辑物体。
- 严格按照每个象限自己的 Mask 工作，不得把一个象限的修改传播到其他象限。
- 禁止添加任何编号、数字、标签、角标、边框、分隔线、水印、说明文字或 UI 元素。
- “左上、右上、左下、右下、轮次、全局”等控制词只用于理解任务，绝对不能画入图片。

四个结果必须在同一次生成中完成，只输出干净的四宫格图像。"""
